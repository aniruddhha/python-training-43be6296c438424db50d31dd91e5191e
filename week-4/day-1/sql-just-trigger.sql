...
drop trigger `ac_dtl_txn_update_trigger`;
drop trigger `ac_dtl_txn_insert_trigger`;

delimiter $$
create trigger `ac_dtl_txn_update_trigger` 
after update on `account_details` for each row 
begin
	declare dt date;
...
delimiter $$
create trigger `ac_dtl_txn_insert_trigger` 
